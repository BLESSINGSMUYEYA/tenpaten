...
const faqs: FAQItem[] = [
    ...
    },
    {
        ...
        category: "Application",
        icon: <ClipboardList className="w-4 h-4 text-brand-primary" />
    },
    {
    ...
    },
    {
        ...
        category: "Affiliate",
        icon: <Share2 className="w-4 h-4 text-brand-primary" />
    },
    {
        ...
        category: "Support",
        icon: <MessageCircle className="w-4 h-4 text-brand-primary" />
    }
];
...
export default function DashboardFAQs() {
    ...
    return (
        <div className="w-full min-w-0 bg-white/80 backdrop-blur-xl rounded-2xl border border-gray-200/50 shadow-xl overflow-hidden h-full flex flex-col group/container hover:border-brand-accent/30 transition-all duration-500">
            <div className="p-6 border-b border-gray-100 bg-linear-to-br from-gray-50/80 to-white/80 flex-shrink-0 space-y-5 relative overflow-hidden">
                {/* Decorative background element */}
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-brand-accent/5 rounded-full blur-3xl group-hover/container:bg-brand-accent/10 transition-colors duration-700" />
                
                <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 rounded-xl bg-brand-primary border border-brand-accent/30 shadow-lg shadow-brand-primary/20">
                ...
                <div className="relative group/search z-10">
                    <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within/search:text-brand-accent transition-colors" />
                    <input
                        type="text"
                        placeholder="Search for help..."
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full pl-10 pr-4 py-2.5 h-11 bg-white border border-gray-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-brand-accent/10 focus:border-brand-accent transition-all placeholder:text-gray-400 shadow-sm"
                    />
                </div>
                        ...
                        return (
                            <div
                                ...
                                className={`rounded-2xl transition-all duration-500 border overflow-hidden ${isOpen
                                    ? 'bg-white border-brand-accent/20 shadow-xl shadow-gray-200/50 scale-[1.02]'
                            ...
                            >
                                ...
                                >
                                    <span className={`mt-0.5 flex-shrink-0 p-2 rounded-lg transition-colors duration-300 ${isOpen ? 'bg-brand-primary text-brand-accent' : 'bg-gray-100 text-brand-primary group-hover/faq:bg-brand-primary/5'}`}>
                                        {faq.icon}
                                    </span>
                                    <span className="flex-1 text-[15px] font-bold text-gray-800 leading-snug pt-1">
                                        {faq.question}
                                    </span>
                                    <span className={`mt-1 flex-shrink-0 p-1 rounded-full transition-all duration-300 ${isOpen ? 'bg-brand-accent text-white' : 'text-gray-400'}`}>
                                        <ChevronDown
                                ...
                                {isOpen && (
                                    <div className="px-5 pb-5 pt-0 ml-[52px]">
                                        <div className="h-px bg-linear-to-r from-gray-100 to-transparent mb-4" />
            ...
            <div className="p-5 bg-white border-t border-gray-100 flex-shrink-0 flex items-center gap-3">
                <a
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-200 rounded-xl text-xs font-black text-gray-700 hover:border-brand-accent hover:text-brand-accent transition-all duration-300 shadow-sm"
                >
                ...
                <a
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-brand-primary border border-transparent rounded-xl text-xs font-black text-white hover:bg-[#2e2f5d] transition-all duration-300 shadow-lg shadow-brand-primary/20 group/contact"
                >
                    <MessageCircle className="w-3.5 h-3.5 text-brand-accent group-hover:rotate-12 transition-transform" />
                    CONTACT US
                </a>
            </div>
        </div>
