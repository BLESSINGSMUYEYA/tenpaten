    ...
    return (
        <div className="rounded-3xl bg-white border border-gray-100 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-xs font-black text-[#36335e] uppercase tracking-[0.2em] opacity-60">Profile Strength</h3>
                <div className="text-base font-black text-[#d5a22d] bg-[#d5a22d]/10 px-3 py-1.5 rounded-lg border border-[#d5a22d]/20 transition-all hover:scale-110 active:scale-95 shadow-sm">
            ...
            <div className="mb-6">
                <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div 
                        className="h-full bg-gradient-to-r from-[#36335e] to-[#d5a22d] transition-all duration-1000" 
            ...
            <div className="space-y-4">
                {allFields.slice(0, 4).map((field, i) => (
                    <div key={i} className="flex items-center justify-between py-1 px-1">
                        <div className="flex items-center gap-3 text-sm text-[#36335e] font-medium tracking-tight">
                            {field.completed ? (
                        ...
                        {!field.completed && (
                             <Link href={field.link} className="text-[#36335e] font-black uppercase tracking-widest text-[10px] hover:text-[#d5a22d] transition-colors bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 shadow-sm active:scale-95">
                                Add
                            </Link>
                        )}
                    </div>
            ...
            <Link 
                ...
                className="mt-6 flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-slate-50 text-[#36335e] text-[10px] font-black uppercase tracking-widest hover:bg-[#36335e] hover:text-white transition-all shadow-sm border border-slate-100"
            >
                Complete Profile
...
export async function UpcomingDeadlines() {
    ...
    return (
        <div className="rounded-3xl bg-white border border-gray-100 p-6 shadow-sm">
                    ...
                    {deadlines.slice(0, 3).map((task) => (
                        <div key={task.id} className="group flex gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <h4 className="text-[11px] font-black text-[#36335e] truncate mb-0.5 tracking-tight">{task.title}</h4>
            ...
            <button className="mt-6 w-full text-center text-[10px] font-black text-slate-300 uppercase tracking-widest hover:text-[#36335e] transition-colors">
                View Calendar
            </button>
        </div>
    );
