...
export default function UniversityCard({ university, matchingProgram, featureMode, layout = 'vertical', onToggleCompare, isComparing }: UniversityCardProps) {
    ...
    if (featureMode) {
        return (
            <Link href={`/dashboard/schools/${university.id}`} className="group relative overflow-hidden rounded-[2.5rem] bg-brand-primary border border-white/5 hover:border-brand-accent/50 p-5 flex flex-col items-center text-center gap-4 transition-all duration-500 hover:shadow-2xl hover:shadow-brand-primary/20 h-full">
                ...
                <div className="space-y-2 flex-1 flex flex-col items-center">
                    <div className="inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-brand-accent text-brand-primary text-[8px] font-black uppercase tracking-widest mb-1">
                        Partner
                    </div>
    ...
    if (layout === 'horizontal') {
        return (
            <>
                ...
                <div className="hidden md:flex group relative overflow-hidden rounded-[2.5rem] bg-white border border-gray-100 hover:border-brand-accent/30 shadow-sm hover:shadow-xl hover:shadow-brand-primary/5 transition-all duration-500 flex-row items-stretch p-6 gap-8">
                    {/* Logo Section */}
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-4">
                        <div className="flex items-start justify-between gap-4">
                            <div className="space-y-1.5">
                                <h3 className="text-lg font-black text-brand-primary group-hover:text-brand-accent transition-colors font-heading capitalize tracking-tight leading-tight">
                                ...
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">
                                    ...
                                    <span className="text-brand-primary/80">{university.programCount} programs available</span>
                                </div>
                            </div>
                                    ...
                                    <button
                                        ...
                                        className={`p-2.5 rounded-xl border transition-all active:scale-90 ${
                                            isComparing 
                                                ...
                                                : 'bg-white border-gray-200 text-gray-400 hover:text-brand-accent hover:border-brand-accent'
                                        }`}
                        ...
                        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
                                ...
                                <Link
                                    ...
                                    className={`flex-1 w-full rounded-2xl p-4 flex items-center justify-between gap-4 transition-all group/match_h ${
                                        isLiteMode 
                                            ...
                                            : 'bg-brand-primary/5 border border-brand-primary/10 hover:bg-brand-primary hover:text-white'
                                    }`}
                                >
                                        ...
                                        {matchingProgram.baseTuition ? (
                                            matchingProgram.scholarshipPercentage && matchingProgram.scholarshipPercentage > 0 ? (
                                                <div className="flex flex-col items-end">
                                                    <span className="text-xs font-black text-brand-accent">{(matchingProgram.baseTuition * (1 - matchingProgram.scholarshipPercentage / 100)).toLocaleString()}</span>
                                                    <span className="text-[8px] font-black text-white/40 line-through">{matchingProgram.baseTuition.toLocaleString()}</span>
                                                </div>
                                            ) : (
                                                <p className="text-[11px] font-black text-brand-accent group-hover/match_h:text-white uppercase tracking-tighter">{matchingProgram.baseTuition.toLocaleString()}</p>
                                            )
                                        ) : (
                                            <p className="text-[11px] font-black text-brand-accent group-hover/match_h:text-white uppercase tracking-tighter">N/A</p>
                                        )}
                                    </div>
                                </Link>
                            ) : (
                            ...
                            <div className="flex flex-wrap gap-2 shrink-0">
                                <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-accent/10 text-brand-accent border border-brand-accent/20 text-[9px] font-black uppercase tracking-[0.2em]">
                                    <ShieldCheck className="w-3 h-3" />
                                    Verified
                                </div>
                                <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-primary/5 text-brand-primary/60 border border-gray-100 text-[9px] font-black uppercase tracking-[0.2em]">
                        ...
                        <div className="mt-auto pt-2 flex flex-wrap items-center justify-between gap-4">
                            ...
                            <Link
                                ...
                                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-primary text-white text-[10px] font-black uppercase tracking-widest hover:bg-brand-accent hover:text-brand-primary transition-all shadow-lg shadow-brand-primary/10 group-hover:translate-x-1"
                            >
                                Quick View
    ...
    return (
        <div className="group relative overflow-hidden rounded-[2.5rem] bg-white border border-gray-100 hover:border-brand-accent/30 shadow-sm hover:shadow-2xl hover:shadow-brand-primary/5 transition-all duration-500 flex flex-col">
            <div className="flex-1 p-6 sm:p-8 flex flex-col gap-5 relative">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="relative w-16 h-16 rounded-2xl bg-white shadow-sm border border-gray-100 flex items-center justify-center p-3 shrink-0 group-hover:scale-105 transition-transform duration-500">
                            {university.logo ? (
                                ...
                                <Building2 className="w-8 h-8 text-brand-primary/40" />
                            )}
                        </div>
                ...
                {matchingProgram ? (
                    <Link
                        href={`/dashboard/programs/${matchingProgram.id}`}
                        className="bg-linear-to-br from-brand-primary to-brand-primary-hover rounded-[2rem] p-5 border border-white/10 flex flex-col items-start justify-between gap-4 animate-in fade-in slide-in-from-right-4 duration-700 hover:shadow-2xl hover:shadow-brand-primary/20 transition-all group/match relative overflow-hidden"
                    >
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                            <div className="w-10 h-10 bg-white/10 backdrop-blur-md text-brand-accent rounded-2xl flex items-center justify-center shadow-inner border border-white/20 group-hover/match:rotate-6 transition-transform">
                                <GraduationCap className="w-5 h-5" />
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-brand-accent uppercase tracking-[0.3em] mb-0.5">Matching Program</p>
                                <p className="text-sm font-black text-white leading-tight group-hover/match:text-brand-accent transition-colors">{matchingProgram.name}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-start gap-1 shrink-0 text-left relative z-10">
                            {matchingProgram.baseTuition && (
                                <div className="flex items-baseline gap-2">
                                    ...
                                    <span className="text-base font-black text-brand-accent tracking-tight">
                                        {matchingProgram.scholarshipPercentage && matchingProgram.scholarshipPercentage > 0 
                            ...
                            <div className="flex gap-3 text-[9px] text-white/60 font-black uppercase tracking-[0.2em] bg-white/5 px-3 py-1 rounded-full">
                                ...
                                {matchingProgram.duration && matchingProgram.intake && <span className="text-brand-accent">•</span>}
