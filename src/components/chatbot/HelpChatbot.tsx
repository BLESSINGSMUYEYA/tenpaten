...
export default function HelpChatbot() {
    ...
    return (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[100] flex flex-col items-end">
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="mb-4 w-[calc(100vw-2rem)] sm:w-[400px] h-[500px] max-h-[calc(100vh-120px)] bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col"
                    >
                        {/* Header */}
                        <div className="p-5 bg-linear-to-br from-[#1d1b41] to-[#2a1b41] text-white flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-white/60 font-medium tracking-widest uppercase flex items-center gap-1">
                                        <Sparkles className="w-2 h-2 text-brand-accent" /> Online Help
                                    </p>
                                </div>
                            </div>
                                    ...
                                    >
                                        <div className={`max-w-[85%] px-4 py-3 rounded-[1.5rem] text-sm shadow-sm ${
                                            isAI 
                                                ...
                                                : 'bg-linear-to-br from-[#1d1b41] to-[#2a1b41] text-white rounded-tr-none'
                                        }`}>
                                ...
                                <div className="flex items-start gap-2">
                                    <div className="bg-white px-4 py-3 rounded-[1.5rem] rounded-tl-none border border-gray-100 shadow-sm">
                                        <div className="flex gap-1">
                                            ...
                                            <div className="w-1.5 h-1.5 bg-brand-accent rounded-full animate-bounce delay-100" />
                                            <div className="w-1.5 h-1.5 bg-brand-accent rounded-full animate-bounce delay-200" />
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="p-4 bg-white border-t border-gray-100">
                            <form onSubmit={handleSend} className="flex gap-2">
                                <input 
                                    ...
                                    className="flex-1 bg-gray-50 border border-transparent focus:border-brand-accent/30 focus:bg-white focus:ring-4 focus:ring-brand-accent/5 rounded-2xl px-4 py-3 text-sm focus:outline-none transition-all placeholder:text-gray-400 font-medium"
                                />
                                <button 
                                    type="submit"
                                    ...
                                    className={`p-3 rounded-2xl shadow-lg transition-all active:scale-95 ${
                                        !input.trim() || isLoading 
                                            ...
                                            : 'bg-brand-accent text-white hover:bg-[#b89531] shadow-brand-accent/20'
                                    }`}
                                >
            ...
            >
                {isOpen ? <X className="w-6 h-6" /> : <MessageSquare className="w-6 h-6 text-brand-accent" />}
                {!isOpen && (
                    <div className="absolute -top-1 -right-1 w-4 h-4 bg-brand-accent border-2 border-white rounded-full animate-bounce" />
                )}
            </motion.button>
        </div>
    );
