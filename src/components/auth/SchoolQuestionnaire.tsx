...
import { ChevronRight, ChevronLeft, CheckCircle2, Building2, Sparkles, ArrowRight } from 'lucide-react';
...
interface SchoolQuestionnaireProps {
    questions: QuestionData[];
    onComplete: (data: Record<string, string>) => void;
}

export default function SchoolQuestionnaire({ questions, onComplete }: SchoolQuestionnaireProps) {
        ...
        return (
            <div className="w-full max-w-md mx-auto space-y-6 p-5 sm:p-6 bg-white rounded-2xl shadow-sm border border-gray-100 text-center">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">We've captured your institution's details. Create your account to get started on the Tenpaten Apply partner network.</p>
                </div>
                <div className="space-y-2 text-left pt-2">
                    {questions.map((q) => {
                        const opts = (q.options as QuestionOption[] | null) ?? [];
                        const ans = answers[q.key];
                        const label = opts.find(o => o.value === ans)?.label ?? ans ?? 'Not answered';
                        return (
                            <div key={q.id} className="p-3 bg-gray-50 rounded-xl border border-gray-100 flex items-start gap-3">
                ...
                <Button
                    variant="ghost"
                    onClick={handleBack}
                    className="w-full h-10 text-sm text-gray-400 hover:text-gray-700"
                >
    ...
    return (
        <div className="w-full max-w-md mx-auto space-y-6 sm:space-y-8 p-5 sm:p-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {/* Question */}
            <div className="min-h-[220px] sm:min-h-[280px] flex flex-col justify-center animate-in fade-in slide-in-from-right-4 duration-300" key={currentStep}>
            ...
            <div className="flex gap-4 pt-4 border-t border-gray-100">
                <Button
                    ...
                    className="flex-1 h-11 rounded-xl border-2 border-gray-100"
                >
