...
export function DashboardCard({ children, className, ...props }: DashboardCardProps) {
    return (
        <div
            ...
            {...props}
        >
            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-accent/10 rounded-full blur-3xl -translate-y-12 translate-x-8 mix-blend-multiply pointer-events-none" />
            {children}
        </div>
    );
}
