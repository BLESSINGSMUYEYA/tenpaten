    ...
    if (!application) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                <h2 className="text-xl font-black text-brand-primary">Application not found</h2>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <h1 className="text-4xl font-black text-brand-primary tracking-tight">{application.prospect.fullName}</h1>
                        <p className="text-gray-500 mt-1 font-medium italic flex items-center gap-2">
                            ...
                            {application.program.name} • <span className="text-brand-primary font-bold">{application.program.university.name}</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className={`h-12 px-6 rounded-2xl text-[10px] font-black uppercase tracking-[0.15em] flex items-center gap-2 shadow-md border
                        ${application.status === 'REJECTED' ? 'bg-red-50 text-red-600 border-red-100' :
                            application.status === 'ENROLLED' ? 'bg-brand-primary text-brand-accent border-brand-primary' :
            ...
            <div className="grid gap-10 lg:grid-cols-3">
                <div className="lg:col-span-2 space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Card className="border-none shadow-xl shadow-brand-primary/5 rounded-[2.5rem] overflow-hidden bg-white border border-gray-50">
                            ...
                            <CardContent className="p-8 space-y-5">
                                <div className="flex justify-between items-center group">
                                    ...
                                    <span className="text-sm font-bold text-brand-primary">{application.prospect.email}</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-sm font-black text-brand-primary uppercase tracking-tight">{personalInfo.nationality || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Passport ID</span>
                                    <span className="text-sm font-mono font-bold text-brand-accent bg-brand-accent/5 px-2 py-1 rounded-lg border border-brand-accent/10">{personalInfo.passportNumber || 'N/A'}</span>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border-none shadow-xl shadow-brand-primary/5 rounded-[2.5rem] overflow-hidden bg-white border border-gray-50">
                            <CardHeader className="bg-gray-50/30 border-b border-gray-50 p-8 pb-4">
                                <CardTitle className="text-[10px] font-black flex items-center gap-3 text-brand-accent uppercase tracking-widest">
                            ...
                            <CardContent className="p-8 space-y-5">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-sm font-black text-brand-primary uppercase tracking-tight">{academicInfo.highestQualification || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-2xl font-black text-brand-primary tracking-tighter">{academicInfo.gpa || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                    ...
                    <Card className="border-none shadow-2xl shadow-brand-primary/5 rounded-[2.5rem] overflow-hidden bg-white">
                        <CardHeader className="bg-gray-50/50 border-b border-gray-100 p-8">
                            <CardTitle className="text-xl font-black flex items-center gap-3 text-brand-primary">
                            ...
                            {documents.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    {documents.map((doc: any, index: number) => (
                                        <div key={index} className="flex items-center justify-between p-6 rounded-[2rem] bg-gray-50/50 border border-gray-100 hover:bg-white hover:shadow-xl hover:shadow-brand-primary/10 transition-all group">
                                            <div className="flex items-center gap-5">
                                                <div className="h-14 w-14 bg-white rounded-2xl flex items-center justify-center text-brand-primary shadow-sm group-hover:bg-brand-primary group-hover:text-brand-accent transition-all">
                                                    <FileText className="w-7 h-7" />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-black text-brand-primary truncate max-w-[120px]">{doc.name || 'Evidence File'}</p>
                                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">{doc.type || 'Asset'}</p>
                                                </div>
                                            </div>
                                            <Button size="icon" variant="ghost" className="h-12 w-12 rounded-2xl hover:bg-brand-primary hover:text-brand-accent transition-all" asChild>
                ...
                <div className="space-y-10">
                    ...
                    <Card className="border-none shadow-2xl shadow-brand-primary/30 rounded-[2.5rem] bg-brand-primary text-white overflow-hidden relative group">
                        <div className="absolute top-0 right-0 w-40 h-40 bg-brand-accent/20 rounded-full -translate-y-16 translate-x-16 blur-3xl group-hover:scale-150 transition-transform duration-700" />
                        <CardHeader className="p-8 pb-4 relative z-10">
                            <CardTitle className="text-[10px] font-black text-brand-accent uppercase tracking-[0.2em] flex items-center gap-3">
                        ...
                        <CardContent className="p-8 pt-0 space-y-8 relative z-10">
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-1.5">
                                    <p className="text-brand-accent text-[10px] font-black uppercase tracking-widest">Fees</p>
                                ...
                                <div className="space-y-1.5">
                                    <p className="text-brand-accent text-[10px] font-black uppercase tracking-widest">Timeline</p>
                    ...
                    {/* Operational Log */}
                    <Card className="border-none shadow-xl shadow-brand-primary/5 rounded-[2.5rem] overflow-hidden bg-white border border-gray-100">
                        <CardHeader className="bg-gray-50/50 border-b border-gray-100 p-8">
                            <CardTitle className="text-sm font-black flex items-center gap-3 text-brand-primary uppercase tracking-widest">
                                ...
                                {application.statusHistory.map((history, idx) => (
                                    <div key={history.id} className="relative pl-12 group">
                                        <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white shadow-xl transition-all duration-500
                                            ${idx === 0 ? 'bg-brand-primary ring-8 ring-brand-primary/5 scale-110' : 'bg-gray-200 group-hover:bg-brand-accent'}`} />
                                        <div>
                                            ...
                                            <p className={`text-sm font-black tracking-tight ${idx === 0 ? 'text-brand-primary' : 'text-gray-600'}`}>{history.status.replace('_', ' ')}</p>
                                            {history.note && (
                                                <div className="mt-3 text-xs bg-brand-primary/5 p-4 rounded-2xl text-brand-primary font-bold italic border border-brand-primary/10 leading-relaxed shadow-sm">
                                                    "{history.note}"
                                                </div>
                                            )}
                                        </div>
