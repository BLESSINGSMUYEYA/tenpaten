            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                    ...
                    <Select value={status} onValueChange={setStatus}>
                        <SelectTrigger id="status" className="w-full h-14 bg-gray-50 border-gray-100 rounded-2xl focus:ring-brand-accent/20 focus:border-brand-accent/30 font-bold text-brand-primary">
                        ...
                        <SelectContent className="rounded-2xl border-gray-100 shadow-2xl">
                            <SelectGroup>
                                ...
                                {statuses.map((s) => (
                                    <SelectItem key={s} value={s} className="py-3 px-4 focus:bg-brand-primary/5 rounded-xl">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-2.5 h-2.5 rounded-full shadow-sm
                                                ${s === 'REJECTED' ? 'bg-red-500' :
                                                    s === 'ENROLLED' ? 'bg-brand-primary' :
                                                        s === 'OFFER_ISSUED' ? 'bg-brand-accent' : 'bg-blue-500'}`}
                                            />
                                            <span className="font-bold text-brand-primary">{s.replace('_', ' ')}</span>
                                        </div>
                                    </SelectItem>
                                ))}
                            </SelectGroup>
                    ...
                    <p className="text-[10px] text-gray-400 flex items-center gap-1.5 mt-2 font-medium">
                        <AlertCircle className="w-3.5 h-3.5 text-brand-accent" />
                ...
                <div className="space-y-3">
                    ...
                    <Textarea
                        ...
                        className="min-h-[100px] bg-gray-50 border-gray-100 focus:bg-white focus:border-brand-accent/30 focus:ring-brand-accent/20 rounded-2xl text-sm font-medium p-4 transition-all"
                    />
                </div>
            </div>
