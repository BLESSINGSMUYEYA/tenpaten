...
export default async function SchoolApplicationDetails({
    ...
    const getStatusStyles = (status: string) => {
        ...
        return 'bg-[#36335e] text-white border-[#36335e] shadow-[#36335e]/20';
    };
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000 pb-20 px-4 sm:px-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 p-8 sm:p-12 bg-white rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/50 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-80 h-80 bg-[#d5a22d]/5 rounded-full blur-[100px] -mr-40 -mt-40" />
                
                <div className="relative z-10 space-y-8">
                    <Link
                        href={`/dashboard/school/applications?panel=${id}`}
                        className="inline-flex items-center gap-2.5 text-slate-400 hover:text-[#36335e] transition-all group text-[11px] font-black uppercase tracking-[0.2em]"
                    >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-8">
                        <div className="w-24 h-24 rounded-[2.5rem] bg-[#36335e] flex items-center justify-center text-[#d5a22d] text-4xl font-black shadow-2xl shadow-[#36335e]/30 border-4 border-white">
                            {application.prospect.fullName.charAt(0)}
                        </div>
                        <div className="space-y-2">
                            <h1 className="text-4xl sm:text-5xl font-black text-[#36335e] tracking-tight leading-[0.9]">
                                {application.prospect.fullName}
                            </h1>
                            <div className="flex flex-wrap items-center gap-6 text-sm font-bold text-slate-400">
                                <span className="flex items-center gap-2.5"><Mail className="w-4.5 h-4.5 text-[#d5a22d]" /> {application.prospect.email}</span>
                                <span className="hidden sm:inline text-slate-200">|</span>
                                <span className="flex items-center gap-2.5 uppercase tracking-widest text-xs font-black"><MapPin className="w-4.5 h-4.5 text-[#d5a22d]" /> {application.id.slice(-8)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative z-10 flex flex-wrap items-center gap-4">
                    <InitiateMessage
                        ...
                        className="flex items-center gap-3 px-8 py-4.5 bg-white border border-slate-100 text-[#36335e] rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#36335e] hover:text-white transition-all shadow-xl shadow-[#36335e]/5 group active:scale-95"
                    />
            ...
            {/* Lifecycle Pipeline */}
            <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl shadow-[#36335e]/5 p-8 sm:p-12 overflow-x-auto custom-scrollbar">
                <div className="min-w-[900px]">
                    <div className="mb-12 flex items-center justify-between">
                        <div>
                            <h2 className="text-3xl font-black text-[#36335e] tracking-tight mb-2">Lifecycle Pipeline</h2>
                            <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">Admissions Milestone Tracker</p>
                        </div>
                        <div className="text-[11px] font-black text-[#d5a22d] bg-[#d5a22d]/10 px-5 py-2.5 rounded-full uppercase tracking-[0.2em] border border-[#d5a22d]/20 shadow-sm">
            ...
            <div className="flex flex-col lg:flex-row gap-10">
                ...
                <div className="flex-1 space-y-10">
                    <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden group">
                        <div className="p-8 sm:p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                            <h2 className="text-2xl font-black text-[#36335e] tracking-tight flex items-center gap-5">
                                <div className="w-12 h-12 rounded-2xl bg-[#d5a22d]/10 flex items-center justify-center shadow-sm">
                        ...
                        <div className="p-8 sm:p-10 grid grid-cols-1 sm:grid-cols-2 gap-8">
                            {[
                                ...
                                <div key={idx} className="p-6 bg-slate-50/50 rounded-2xl border border-slate-50 group-hover:bg-white group-hover:border-slate-100 transition-all duration-300">
                                    ...
                                    <p className="text-[15px] font-black text-[#36335e] leading-tight">{item.value}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                        ...
                        section.data && (
                            <div key={section.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden group">
                                <div className="p-8 sm:p-10 border-b border-slate-50 flex items-center gap-5 bg-slate-50/30">
                                    <div className="w-12 h-12 rounded-2xl bg-[#36335e]/5 flex items-center justify-center group-hover:bg-[#36335e] transition-all duration-500">
                                        <section.icon className="w-6 h-6 text-[#36335e] group-hover:text-[#d5a22d] transition-all" />
                                    </div>
                                    <h2 className="text-2xl font-black text-[#36335e] tracking-tight">{section.label}</h2>
                                </div>
                                <div className="p-8 sm:p-10 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                                    {Object.entries(section.data).map(([key, value]: [string, any]) => (
                                        <div key={key} className="space-y-3">
                                            ...
                                            <div className="text-[15px] text-[#36335e] font-black p-6 bg-slate-50/50 rounded-2xl border border-slate-50 group-hover:bg-white group-hover:border-slate-100 transition-all shadow-sm">
                ...
                <div className="lg:w-[420px] space-y-10">
                    ...
                    <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden sticky top-10">
                        <div className="p-10 border-b border-slate-100 bg-[#36335e] text-white relative">
                            <div className="absolute top-0 right-0 w-24 h-24 bg-[#d5a22d]/10 rounded-full blur-2xl -mr-12 -mt-12" />
                            <h2 className="text-2xl font-black tracking-tight leading-none relative z-10">Decision Control</h2>
                            <p className="text-[10px] font-black text-[#d5a22d] uppercase tracking-[0.4em] mt-3 relative z-10">Executive Actions</p>
                        </div>
