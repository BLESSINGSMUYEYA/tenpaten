...
export default async function Page() {
    ...
    if (!affiliate) {
        return (
            <div className="w-full flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-3xl border-2 border-dashed border-gray-100">
                ...
                <h1 className="text-2xl font-black text-brand-primary tracking-tight mb-2">Partner Portal</h1>
    ...
    const statCards = [
        ...
        { label: 'Estimated Earnings', value: `$${estimatedEarnings}`, icon: DollarSign, color: 'from-brand-accent/20 to-[#b08523]/20', iconColor: 'text-brand-accent' },
    ];
    ...
    return (
        <div className="w-full space-y-5 pb-6 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-accent/10 text-brand-accent text-[10px] font-black uppercase tracking-[0.2em] mb-2">
                        <TrendingUp className="w-3 h-3" />
                        Affiliate Partner
                    </div>
                    <h1 className="text-2xl font-black text-brand-primary tracking-tight">Partner Dashboard</h1>
                    <p className="text-slate-500 mt-1 font-medium text-sm">
                        Managing referrals as <span className="font-black text-brand-accent">{affiliate.referralCode}</span>
                    </p>
                </div>
            ...
            <div className="bg-linear-to-br from-brand-primary to-brand-primary-hover rounded-2xl shadow-xl shadow-brand-primary/20 p-5 md:p-6 text-white relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-80 h-80 bg-brand-accent/10 rounded-full blur-3xl -mr-40 -mt-40 pointer-events-none group-hover:bg-brand-accent/20 transition-colors duration-1000" />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {statCards.map((stat) => {
                    ...
                    return (
                        <div key={stat.label} className={`bg-linear-to-br ${stat.color} rounded-2xl p-5 border border-white/5 shadow-sm space-y-3`}>
                    ...
                    <div className="space-y-2">
                        {[
                            ...
                            <div key={row.label} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100">
                                ...
                                <span className="text-xs font-black text-brand-accent px-2 py-1 bg-white rounded-lg border border-gray-100">{row.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        {[
                            ...
                            return (
                                <Link
                                    key={action.href}
                                    href={action.href}
                                    className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 hover:border-brand-accent/40 hover:bg-brand-accent/5 transition-all group"
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-brand-primary/5 flex items-center justify-center">
                                            <ActionIcon className="w-4 h-4 text-brand-primary" />
                                        </div>
                                        <div>
                                            <div className="text-sm font-bold text-brand-primary">{action.label}</div>
                                            <div className="text-xs text-slate-400">{action.desc}</div>
                                        </div>
                                    </div>
                                    <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-brand-accent transition-colors" />
                                </Link>
                            );
                        })}
                    </div>
            ...
            {referrals.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                        ...
                        <Link href="/dashboard/affiliate/referrals" className="text-xs font-bold text-brand-accent hover:underline flex items-center gap-1">
                    ...
                    <div className="divide-y divide-gray-50">
                        {referrals.slice(0, 5).map((ref) => (
                            <div key={ref.id} className="flex items-center justify-between px-5 py-3">
                                <div>
                                    <div className="text-sm font-bold text-brand-primary">{ref.prospect.fullName}</div>
